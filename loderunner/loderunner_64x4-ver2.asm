...
ueEnemyMovesLeftToPlayer:
                              ;ldx enemyIndex
            MBZ numberOfEnemies,1 ;ldy numberOfEnemies  -> Y = Anzahl der F채nger
            LDR enemyYBlockPos ;lda enemyYBlockPos,x -> A = Y-Position F채nger
ueCheckEnemyToTheLeft:
            PHS               ;pha -> Y-Position F채nger
            LDI <_enemyYBlockPos PHS JPS loadEnemyYidxA PLS ;     A=enemyYBlockPos,y
            STZ 0
            LDS 1
            CPZ 0             ;cmp enemyYBlockPos,y
            BNE ueNextEnemyToTheLeft
            LDR enemyXBlockPos ;lda enemyXBlockPos,x -> A = X-Position F채nger
                              ;sec
            DEC               ;sbc #$01 -> links neben akuellem F채nger
            PHS
            LDI <_enemyXBlockPos PHS JPS loadEnemyYidxA PLS ;     A=enemyXBlockPos,y
            STZ 0
            PLS
...
ueMoveLe1:    STR enemyXBlkInternalDecimalOffset ;sta enemyXBlkInternalDecimalOffset,x
            JPA ueCheckSpecialCases ;&&&& unneeded - unn철tig
...
lineloop:   LDR spritePtr     ; Spritedaten einer Zeile in den Puffer kopieren
            ...
            LDR spritePtr     ; Pointer auf n채chstes Byte
            STB buffer+1      ; high Byte Sprite (rechte Seite)
            CLB buffer+2      ; Clear Byte (drittes Buffer-Byte 0 f체r Pixelverschiebung)

            MBZ shift,0       ; Puffer verschieben
            DEZ 0             ; Decrement X
            BCC shiftdone     ; Branch on Carry Clear     ; shift that buffer to pixel position (kein verschieben n철tig C=0 wenn shift=0)
...
shiftdone:    LDR vAddr       ; Buffer mit den Videodaten verkn체pfen
            XRB buffer+0
            STR vAddr
            INW vAddr
            LDR vAddr
            ...
            INW spritePtr     ; n채chstes Sprite Datenbyte
            DEB lineCnt
...
playerFrameNumber: 0x00,      ; 0x0f Player f채llt
...
spriteData:                   ;00=cursor block (white)
                ...
                0xa8,0xaa, 0x54,0x55, 0xa8,0xaa, 0x54,0x55, 0xa8,0xaa, 0x54,0x55, 0xa8,0xaa,
                              ;01=brick (red)
brick:          0x9c,0xff, 0x9c,0xff, 0x9c,0xff, 0x9c,0xff, 0x9c,0xff, 0x00,0x00, 0x00,0x00,
                0xfc,0xe7, 0xfc,0xe7, 0xfc,0xe7, 0xfc,0xe7, 0xfc,0xe7, 0x00,0x00, 0x00,0x00,
                              ;02=solid block (red)
solid:          0xfc,0xff, 0xfc,0xff, 0xfc,0xff, 0xfc,0xff, 0xfc,0xff, 0xfc,0xff, 0xfc,0xff,
				0xfc,0xff, 0xfc,0xff, 0xfc,0xff, 0xfc,0xff, 0xfc,0xff, 0x00,0x00, 0x00,0x00,
                              ;03=ladder (white)
